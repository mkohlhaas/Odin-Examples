		...
		for i in 0 ..< GRID_HORIZONTAL_SIZE {
			...
			case .Empty:
				...
				rl.DrawLine(offset.x +
					SQUARE_SIZE, offset.y, offset.x + SQUARE_SIZE, offset.y + SQUARE_SIZE, rl.LIGHTGRAY)
				rl.DrawLine(offset.x, offset.y +
					SQUARE_SIZE, offset.x + SQUARE_SIZE, offset.y + SQUARE_SIZE, rl.LIGHTGRAY)
			case .Full:
...
get_random_piece :: proc() {
	...
	switch random {
	case 0:
		incoming_piece[1][1] = .Moving;incoming_piece[2][1] = .Moving;incoming_piece[1][2] = .Moving;incoming_piece[2][2] =
		.Moving //Cube
	case 1:
		incoming_piece[1][0] = .Moving;incoming_piece[1][1] = .Moving;incoming_piece[1][2] = .Moving;incoming_piece[2][2] =
		.Moving //L
	case 2:
		incoming_piece[1][2] = .Moving;incoming_piece[2][0] = .Moving;incoming_piece[2][1] = .Moving;incoming_piece[2][2] =
		.Moving //L inversa
	case 3:
		incoming_piece[0][1] = .Moving;incoming_piece[1][1] = .Moving;incoming_piece[2][1] = .Moving;incoming_piece[3][1] =
		.Moving //Recta
	case 4:
		incoming_piece[1][0] = .Moving;incoming_piece[1][1] = .Moving;incoming_piece[1][2] = .Moving;incoming_piece[2][1] =
		.Moving //Creu tallada
	case 5:
		incoming_piece[1][1] = .Moving;incoming_piece[2][1] = .Moving;incoming_piece[2][2] = .Moving;incoming_piece[3][2] =
		.Moving //S
	case 6:
		incoming_piece[1][2] = .Moving;incoming_piece[2][2] = .Moving;incoming_piece[2][1] = .Moving;incoming_piece[3][1] =
		.Moving //S inversa
	}
}
...
resolve_turn_movement :: proc() -> bool {
	...
	if rl.IsKeyDown(.UP) {
		...
		if ((grid[piece_position.x + 3][piece_position.y] == .Moving) && (grid[piece_position.x][piece_position.y] !=
		   .Empty) && (grid[piece_position.x][piece_position.y] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 3][piece_position.y + 3] == .Moving) && (grid[piece_position.x +
                                                                                    3][piece_position.y] !=
		   .Empty) && (grid[piece_position.x + 3][piece_position.y] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x][piece_position.y + 3] == .Moving) && (grid[piece_position.x +
                                                                                3][piece_position.y +
                                                                                   3] != .Empty) &&
		   (grid[piece_position.x + 3][piece_position.y + 3] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x][piece_position.y] == .Moving) && (grid[piece_position.x][piece_position.y +
                                                                                              3] !=
		   .Empty) && (grid[piece_position.x][piece_position.y + 3] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 1][piece_position.y] == .Moving) && (grid[piece_position.x][piece_position.y +
                                                                                                  2] !=
		   .Empty) && (grid[piece_position.x][piece_position.y + 2] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 3][piece_position.y + 1] == .Moving) && (grid[piece_position.x +
                                                                                    1][piece_position.y] !=
		   .Empty) && (grid[piece_position.x + 1][piece_position.y] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 2][piece_position.y + 3] == .Moving) && (grid[piece_position.x +
                                                                                    3][piece_position.y +
                                                                                       1] !=
		   .Empty) && (grid[piece_position.x + 3][piece_position.y + 1] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x][piece_position.y + 2] == .Moving) && (grid[piece_position.x +
                                                                                2][piece_position.y +
                                                                                   3] != .Empty) &&
		   (grid[piece_position.x + 2][piece_position.y + 3] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 2][piece_position.y] == .Moving) && (grid[piece_position.x][piece_position.y +
                                                                                                  1] !=
		   .Empty) && (grid[piece_position.x][piece_position.y + 1] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 3][piece_position.y + 2] == .Moving) && (grid[piece_position.x +
                                                                                    2][piece_position.y] !=
		   .Empty) && (grid[piece_position.x + 2][piece_position.y] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 1][piece_position.y + 3] == .Moving) && (grid[piece_position.x +
                                                                                    3][piece_position.y +
                                                                                       2] !=
		   .Empty) && (grid[piece_position.x + 3][piece_position.y + 2] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x][piece_position.y + 1] == .Moving) && (grid[piece_position.x +
                                                                                1][piece_position.y +
                                                                                   3] != .Empty) &&
		   (grid[piece_position.x + 1][piece_position.y + 3] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 1][piece_position.y + 1] == .Moving) && (grid[piece_position.x +
                                                                                    1][piece_position.y +
                                                                                       2] !=
		   .Empty) && (grid[piece_position.x + 1][piece_position.y + 2] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 2][piece_position.y + 1] == .Moving) && (grid[piece_position.x +
                                                                                    1][piece_position.y +
                                                                                       1] !=
		   .Empty) && (grid[piece_position.x + 1][piece_position.y + 1] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 2][piece_position.y + 2] == .Moving) && (grid[piece_position.x +
                                                                                    2][piece_position.y +
                                                                                       1] !=
		   .Empty) && (grid[piece_position.x + 2][piece_position.y + 1] != .Moving)) {
			checker = true
		} else if ((grid[piece_position.x + 1][piece_position.y + 2] == .Moving) && (grid[piece_position.x +
                                                                                    2][piece_position.y +
                                                                                       2] !=
		   .Empty) && (grid[piece_position.x + 2][piece_position.y + 2] != .Moving)) {
			checker = true
		}

		if !checker {
			piece[0][0], piece[3][0], piece[3][3], piece[0][3] = piece[3][0], piece[3][3], piece[0][3],
			piece[0][0]

			piece[1][0], piece[3][1], piece[2][3], piece[0][2] = piece[3][1], piece[2][3], piece[0][2],
			piece[1][0]

			piece[2][0], piece[3][2], piece[1][3], piece[0][1] = piece[3][2], piece[1][3], piece[0][1],
			piece[2][0]

			piece[1][1], piece[2][1], piece[2][2], piece[1][2] = piece[2][1], piece[2][2], piece[1][2],
			piece[1][1]
		}
