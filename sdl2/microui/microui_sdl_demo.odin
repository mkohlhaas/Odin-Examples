...
main :: proc() {
  fmt.println(state.bg)
	...
	state.atlas_texture = SDL.CreateTexture(renderer, u32(SDL.PixelFormatEnum.RGBA32), .TARGET,
...
render :: proc(ctx: ^mu.Context, renderer: ^SDL.Renderer) {
	render_texture :: proc(
		...
		SDL.RenderCopy(renderer, state.atlas_texture, &SDL.Rect{
				src.x,
				src.y,
				src.w,
				src.h,
